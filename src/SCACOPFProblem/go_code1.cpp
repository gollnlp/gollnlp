...
int MyCode1::initialize(int argc, char *argv[])
{
  ...
  //empty the file if any
  if(my_rank==0) {
    FILE* file = fopen("gollnlp_optim.txt", "w+");
    if(NULL==file) {
      printf("[warning[ could not file [gollnlp_optim.txt] \n");
    } else {
      fclose(file);
    }
  }
...
bool MyCode1::do_phase1()
{
  ...
  if(!iAmSolver) {
    ...
    if(true) {
      ...
      arr = new double[sz]; for(int i=0; i<sz; i++) arr[0]=0.;
      MPI_Bcast(arr, sz, MPI_DOUBLE, rank_solver_rank0, comm_world);
      scacopf_prob->duals_bounds_upper()->copy_from(arr);
      delete[] arr;
      ...
      MPI_Bcast(arr, sz, MPI_DOUBLE, rank_solver_rank0, comm_world);
      scacopf_prob->duals_constraints()->copy_from(arr);
      delete[] arr;
    } else {
				     ...
				     const std::vector<int>& K_actions)
{
  ...
  assert(scacopf_prob->use_filelocks_when_writing == false);
  //scacopf_prob->set_quadr_penalty_qg0(true);
